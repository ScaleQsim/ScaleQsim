...
class StateSpaceCUDA :
 ...
 public:
  ...
  void SetStateZero(State& state) const {
    ...
    printf("KCJ: State initialized to |0> with first element set to 1 at address: %p\n", state.get());
      IO::messagef("KCJ: State initialized to |0> with first element set to 1 at address: %p\n", state.get());
     std::cout << "Debug: KCJ" << std::endl;
 ...
 protected:
  ...
  void* AllocScratch(uint64_t size) const {
    ...
    printf("KCJ: Allocated scratch memory of size: %llu at address: %p\n", size, scratch_);
        IO::messagef("KCJ: Allocated scratch memory of size: %llu at address: %p\n", size, scratch_);

...
struct Gate {
  ...
  //KCJ to_string method. 
   std::string to_string() const {
    std::ostringstream oss;
    oss << "Gate kind: " << static_cast<int>(kind) << "\n";
    oss << "Time: " << time << "\n";
    oss << "Qubits: [";
    ...
    oss << "Controlled by: [";
    for (const auto& c : controlled_by) {
      oss << c << " ";
    }
    oss << "]\n";

    oss << "Control mask: " << cmask << "\n";
    ...
    oss << "Unfusible: " << (unfusible ? "true" : "false") << "\n";
    oss << "Swapped: " << (swapped ? "true" : "false") << "\n";

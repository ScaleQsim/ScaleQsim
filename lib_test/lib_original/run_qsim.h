...
struct QSimRunner final {
 public:
  ...
  static bool Run(const Parameter& param, const Factory& factory,
    ...
    //KCJ-Debug-file.
    //std::ofstream debug_file("/pscratch/sd/s/sgkim/qsim/debug_output.txt", std::ios::app);

    StateSpace state_space = factory.CreateStateSpace();
    auto state = state_space.Create(circuit.num_qubits);

    // KCJ: State 정보 출력
    IO::messagef("KCJ: State memory address: %p\n", &state);
    fflush(stdout); 

    unsigned char* byte_ptr = reinterpret_cast<unsigned char*>(&state);
    size_t state_size = sizeof(state);  // 상태 벡터의 전체 크기

    IO::messagef("KCJ: State memory content:\n");
    fflush(stdout); 
    for (size_t i = 0; i < state_size; ++i) {
        IO::messagef("%02X ", static_cast<int>(byte_ptr[i]));  // 16진수로 각 바이트를 출력
        if ((i + 1) % 16 == 0) {
            IO::messagef("\n");  // 16바이트 단위로 줄바꿈
            fflush(stdout); 
        }
    }
    IO::messagef("\nKCJ: This is a test message to see if it appears on the console.\n");
    fflush(stdout); 
